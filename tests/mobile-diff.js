...
const slug = process.argv[2] || 'audiom-demo';
const baselineFile = `tests/baseline/${slug}-mobile.png`;
const currentFile = `tests/current/mobile-${slug}.png`;
const diffFile = `tests/current/diff-mobile-${slug}.png`;
...
(async () => {
  ...
  console.log(`${slug} [mobile]: ${pct}% diff (${w}x${h}, baseline ${b.width}x${b.height}, current ${c.width}x${c.height})`);
